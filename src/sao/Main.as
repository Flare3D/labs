package sao
{
	...
	public class Main extends Sprite 
	{
		...
		private var enabledSAO:Boolean = true;
		...
		public function Main() 
		{
			...
			scene.addChildFromFile( "../assets/map.f3d" );
		}
		...
		private function completeEvent(e:Event):void 
		{
			...
			var bmp:BitmapData = new BitmapData( 512, 512, false );
				bmp.noise( 100 );
			sNoise = new Texture3D( bmp );
		...
		private function updateEvent(e:Event):void 
		{
			...
			if ( Input3D.keyHit( Input3D.NUMBER_1 ) ) enabledSAO = !enabledSAO;
		...
		private function renderEvent(e:Event):void 
		{
			...
			// the SAO should be applied during composition, not here..this is just a shortcut to make it work.
			// it also needs some blur pass before getting into the screen to avoid the noise.
			scene.context.setRenderToBackBuffer();
			fx.setTechnique( enabledSAO ? "sao" : "quad" );
			fx.params.targetBuffer.value = targetBuffer;
			fx.params.gBuffer.value = gBuffer;
			fx.params.sNoise.value = sNoise;
