package gamepad
{
	...
	{
		[Embed(source = "../agal2/assets/fx.flsl.compiled", mimeType = "application/octet-stream")] private var FX:Class;
		[Embed(source = "../agal2/assets/mrt.flsl.compiled", mimeType = "application/octet-stream")] private var MRT:Class;
		[Embed(source = "../agal2/assets/lighting.flsl.compiled", mimeType = "application/octet-stream")] private var LIGHTING:Class;
		...
		private var pad0:VirtualPad;
		private var pad1:VirtualPad;
		...
		public function Main() 
		{
			...
			airGamepad.addEventListener(AIRGamepadErrorEvent.CONNECT_ERROR, gamepadConnectionError );
			airGamepad.addEventListener(AIRGamepadEvent.DISCONNECT, gamepadDisconnectEvent );
			airGamepad.connect( stage, "Flare3D", "http://www.flare3d.com" );
			
			pad0 = new VirtualPad( 50, 50, VirtualPad.Y_AXIS, airGamepad );
			pad1 = new VirtualPad( 50, 50, VirtualPad.X_AXIS + VirtualPad.Y_AXIS, airGamepad );
		...
		private function gamepadDisconnectEvent(e:AIRGamepadEvent):void 
		{
			airGamepad.connect( stage, "Flare3D", "http://www.flare3d.com" );
		}
		...
		private function gamepadConnected(e:AIRGamepadEvent):void 
		{
			...
			var x0:Number = 10;
			var y0:Number = 10;
			var w0:Number = sizeX * 0.25 - 20;
			var h0:Number = sizeY - 20;
			pad0.setSize( x0, y0, w0, h0 );
			
			var x1:Number = sizeX * 0.25 + 20;
			var y1:Number = 10;
			var w1:Number = sizeX * 0.75 - 20;
			var h1:Number = sizeY - 20;
			pad1.setSize( x1, y1, w1, h1 );
			airGamepad.vibrate(500);
			
			var sprite:Sprite = new Sprite;
			sprite.graphics.clear();
			sprite.graphics.beginFill( 0x0 );
			sprite.graphics.drawRect( 0, 0, sizeX, sizeY );
			sprite.graphics.endFill();
			sprite.graphics.beginFill( 0x0 );
			sprite.graphics.lineStyle( sizeX * 0.1, 0xffffff );
			sprite.graphics.drawCircle( x1 + w1 * 0.5, y1 + h1 * 0.5, w1 * 0.3 );
			sprite.graphics.endFill();
			sprite.graphics.beginFill( 0xffffff );
			sprite.graphics.drawCircle( x0 + w0 * 0.7, y0 + h0 * 0.25, w0 * 0.2 );
			sprite.graphics.drawCircle( x0 + w0 * 0.5, y0 + h0 * 0.75, w0 * 0.2 );
			sprite.graphics.endFill();
			airGamepad.drawSprite( sprite );
		}
		
		private function gamepadConnectionError(e:AIRGamepadErrorEvent):void 
		{
		...
		private function updateEvent(e:Event):void 
		{
			...
			if ( Multitouch.supportsTouchEvents || airGamepad ) {
				if ( pad1.isDown ) {
		...
		private function configureBuffers( e:Event = null ):void 
		{
			...
			pad0.setSize( 10, 10, sizeX * 0.25 - 20, sizeY - 20 );
			pad1.setSize( sizeX * 0.25 + 10, 10, sizeX * 0.75 - 20, sizeY - 20 );
		}
	}
}